...
async function updateOrderStatus(request: NextRequest) {
  try {
    console.log('Ã°Å¸â€â€˜ Checking environment variables...')
    ...
    if (!serviceAccountEmail || !privateKey || !spreadsheetId) {
      console.error('Ã¢ÂÅ’ Google Sheets credentials not configured')
    ...
    console.log('Ã°Å¸â€Â Searching for order:', orderId, 'New status:', status)
    ...
    console.log(`Ã°Å¸â€œÅ  Found ${rows.length} rows in Orders sheet`)
    
    // Log all order IDs for debugging
    console.log('Ã°Å¸â€œâ€¹ Available Order IDs:', rows.map((r, i) => `Row ${i+2}: ${r[0]}`))
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (sheetOrderId === searchOrderId) {
        ...
        console.log(`Ã¢Å“â€¦ Found order at row ${rowIndex}`)
        break
      }
    }

    if (rowIndex === -1) {
      console.error('Ã¢ÂÅ’ Order not found:', orderId)
      return NextResponse.json(
    ...
    if (statusColumnIndex === -1) {
      console.error('Ã¢ÂÅ’ Status column not found')
      return NextResponse.json(
    ...
    console.log(`Ã°Å¸â€œÂ Status column index: ${statusColumnIndex} (Letter: ${String.fromCharCode(65 + statusColumnIndex)})`)

    // Convert column index to letter (0=A, 1=B, 2=C, etc.)
    const statusColumnLetter = String.fromCharCode(65 + statusColumnIndex)
    
    console.log(`Ã°Å¸â€œÂ About to update: Row ${rowIndex}, Column ${statusColumnLetter} (${statusColumnIndex + 1}), Value: ${status}`)
    ...
    console.log(`Ã¢Å“â€¦ Order ${orderId} status updated to: ${status}`)
    console.log(`Ã°Å¸â€œÅ  Verification: Updated cell reference: Orders!${statusColumnLetter}${rowIndex}`)
    ...
    console.error("Ã¢ÂÅ’ Error updating order status:", error)
